...
class PDBDownloader:
    ...
    def get_active_rna_ids() -> Set[str]:
        ...
        while True:
            query = {
                "query": {"type": "terminal", "service": "text", "parameters": {"attribute": "entity_poly.rcsb_entity_polymer_type", "operator": "exact_match", "value": "RNA"}},
                "return_type": "entry", "request_options": {"paginate": {"start": start, "rows": rows}}
            }
            try:
                r = requests.post(url, json=query, timeout=30); r.raise_for_status()
                data = r.json(); results = data.get('result_set', [])
                if not results: break
                for res in results: all_ids.add(res['identifier'].upper())
                start += rows
                if len(all_ids) >= data.get('total_count', 0): break
            except: break
        return all_ids
    ...
    def download_cif(pdb_id: str, target_path: str) -> bool:
        ...
        try:
            r = requests.get(f"https://files.rcsb.org/download/{pdb_id.upper()}.cif", timeout=20)
    ...
    def fetch_metadata(pdb_id: str) -> Tuple[str, str, Optional[float], str]:
        ...
        try:
            ...
            with urllib.request.urlopen(url_ent, timeout=5) as resp:
                ...
                src = ent_data.get("rcsb_entity_source_organism", []) or ent_data.get("entity_src_gen", []) or ent_data.get("entity_src_nat", [])
                org = src[0].get("scientific_name") or "Unknown Organism" if src else "Unknown Organism"
            return title[:200], org[:100], res, method
        except: return "Unknown Molecule", "Unknown Organism", None, "Unknown Method"
...
class Utils:
    ...
    def to_int(val: Any) -> Optional[int]:
        ...
        if isinstance(val, dict): return val.get("serial")
        try: return int(val) 
        except: return None 
    ...
    def get_full_sequence_from_data(data_obj: Dict) -> str:
        ...
        def process_strand(strand):
            ...
            if seq and chain and start is not None:
                if chain not in chains: chains[chain] = {}
                for i, char in enumerate(seq): chains[chain][start + i] = char
        
        # Traverse all possible location fields
        for loc in data_obj.get('location', []): process_strand(loc)
        for stem in data_obj.get('context', {}).values():
            if isinstance(stem, dict): process_strand(stem.get('strand5p', {})); process_strand(stem.get('strand3p', {}))
        for ext in data_obj.get('extended_helices', []):
            strands = ext.get('strands', {})
            for s in ['upstream', 'downstream']:
                process_strand(strands.get(s, {}).get('strand5p', {})); process_strand(strands.get(s, {}).get('strand3p', {}))
            for comp in ext.get('components', []):
                for loc in comp.get('location', []): process_strand(loc)
                process_strand(comp.get('internal_stem', {}).get('strand5p', {})); process_strand(comp.get('internal_stem', {}).get('strand3p', {}))
        for s in ['upstream', 'downstream']:
            process_strand(data_obj.get('strands', {}).get(s, {}).get('strand5p', {})); process_strand(data_obj.get('strands', {}).get(s, {}).get('strand3p', {}))
        for c in data_obj.get('components', []):
            for loc in c.get('location', []): process_strand(loc)
            process_strand(c.get('internal_stem', {}).get('strand5p', {})); process_strand(c.get('internal_stem', {}).get('strand3p', {}))
        
        parts = []
        for cid in sorted(chains.keys()):
            nts = chains[cid]; sorted_s = sorted(nts.keys())
            if not sorted_s: continue
            curr, prev = "", None
            for s in sorted_s:
                if prev is not None and s != prev + 1: parts.append(curr); curr = nts[s]
                else: curr += nts[s]
                prev = s
            parts.append(curr)
        return "-".join(parts)
    ...
    def get_closing_indices(motif: Dict) -> Set[int]: 
        ...
        for s in motif.get("strands", []): 
            ...
            if f is not None: indices.add(f) 
            if l is not None: indices.add(l) 
        return indices 
    ...
    def enrich_motif_data(motif: Dict, full_data: Any) -> None: 
        ...
        if not isinstance(full_data, dict) or "bpseq_index" not in full_data: return 
        mapping = full_data["bpseq_index"] 
        strands_to_enrich = []
        if "strands" in motif: strands_to_enrich.extend(motif["strands"])
        elif "location" in motif and "strands" in motif["location"]: strands_to_enrich.extend(motif["location"]["strands"])    
        if "strand5p" in motif: strands_to_enrich.append(motif["strand5p"])
        if "strand3p" in motif: strands_to_enrich.append(motif["strand3p"])
            ...
            for key_name in ["first", "last"]:
                ...
                if raw is not None and not isinstance(raw, dict): 
                    ...
                    if key in mapping: 
                        ...
                        s[key_name] = {"chain": auth["chain"], "number": auth["number"], "icode": clean_icode(auth.get("icode")), "name": auth["name"], "serial": int(key)}
            if "all_serials" in s: del s["all_serials"]

        interactions_details = motif.get("modules", {}).get("interactions", {}).get("details", [])
            ...
            for key in ["nt1", "nt2"]:
                ...
                if raw_nt is not None and not isinstance(raw_nt, dict):
                    ...
                    if key_str in mapping:
                        ...
                        inter[key] = {"chain": auth["chain"], "number": auth["number"], "icode": clean_icode(auth.get("icode")), "name": auth["name"], "serial": int(key_str)}
    ...
    def get_stem_str(stem: Dict) -> str:
        ...
        if "strand5p" in stem and "strand3p" in stem:
            ...
            f5, l5, f3, l3 = s5.get("first", {}), s5.get("last", {}), s3.get("first", {}), s3.get("last", {})
            ...
            if isinstance(f, dict): return f"[{f.get('chain')}{f.get('number')}-{l.get('number')}]"
            return f"[{f}-{l}]"
        return "None"
    ...
    def get_stem_nt_count(stem: Dict) -> int:
        ...
        if "strand5p" in stem and "strand3p" in stem:
            l1 = len(stem["strand5p"].get("sequence", "")) or (abs(Utils.to_int(stem["strand5p"].get("last", 0)) - Utils.to_int(stem["strand5p"].get("first", 0))) + 1)
            l2 = len(stem["strand3p"].get("sequence", "")) or (abs(Utils.to_int(stem["strand3p"].get("last", 0)) - Utils.to_int(stem["strand3p"].get("first", 0))) + 1)
            return l1 + l2
        elif "first" in stem and "last" in stem: return (abs(Utils.to_int(stem["last"]) - Utils.to_int(stem["first"])) + 1) * 2
        return 0
    ...
    def get_helix_unique_nt(motifs_in_helix: List[Dict]) -> int:
        ...
        def add_range(f, l, chain):
            if f is not None and l is not None:
                for i in range(min(f, l), max(f, l) + 1): unique_nt.add((chain, i))
        for m in motifs_in_helix:
            for s in m.get("location", {}).get("strands", []): add_range(Utils.to_int(s.get("first")), Utils.to_int(s.get("last")), s.get("chain"))
            ...
            for key, stem in context.items():
                if "stem" in key or key in ["upstream", "downstream"]:
                    for skey in ["strand5p", "strand3p"]:
                        s = stem.get(skey, {})
                        add_range(Utils.to_int(s.get("first")), Utils.to_int(s.get("last")), s.get("chain"))
...
class Core: 
    ...
    def classify_motif(loop_strands: List[Dict]) -> Optional[str]:
        ...
        if len(loop_strands) == 1:
            s = loop_strands[0]
            if any(c in s.get('structure', '') for c in ['[', ']', '{', '}']): return None
            if '.' in s.get('structure', ''): return "HAIRPIN"
            return None
        if len(loop_strands) == 2:
            for s in loop_strands:
                if any(c in s.get('structure', '') for c in ['[', ']', '{', '}']): return None
            has_dots = [('.' in s.get('structure', '')) for s in loop_strands]
            if has_dots[0] and has_dots[1]: return "INTERNAL_LOOP"
            elif has_dots[0] ^ has_dots[1]: return "BULGE"
            return None
    ...
    def build_pairs_index(data: Dict) -> Dict:
        ...
        for bp in data.get("base_pairs", []):
            try:
                ...
                if not nt1 or not nt2: continue
                l1, l2 = nt1.get("label"), nt2.get("label")
                if not l1 or not l2: continue
                
                s1, s2 = reverse_map.get((l1.get("chain"), l1.get("number"))), reverse_map.get((l2.get("chain"), l2.get("number")))
                if s1 is None or s2 is None: continue
                key = tuple(sorted((s1, s2)))
                index[key] = {"lw": bp.get("lw") or bp.get("LW") or "???", "seq": f"{l1.get('name')}-{l2.get('name')}"}
            except: continue
        return index
    ...
    def find_interactions(loop_strands: List[Dict], pairs_index: Dict) -> List[Dict]: 
        ...
        for i1, i2 in itertools.combinations(sorted(list(loop_indices)), 2): 
            ...
            if key in pairs_index: 
                ...
                if info["lw"] == "cWW" and info["seq"] in ["G-C", "C-G", "A-U", "U-A", "G-U", "U-G"]: continue 
                interactions.append({"nt1": i1, "nt2": i2, "seq": info["seq"], "lw": info["lw"]}) 
        return interactions 
...
class Analysis: 
    ...
    def categorize_interaction(motif: Dict, interactions: List[Dict]) -> Dict[str, int]: 
        ...
        closing_idxs, counts, nt_to_interactions, valid_interactions = Utils.get_closing_indices(motif), {}, {}, [] 
        for i in interactions: 
            n1, n2 = i["nt1"], i["nt2"] 
            if Utils.to_int(n1) in closing_idxs and Utils.to_int(n2) in closing_idxs: continue 
            valid_interactions.append(i) 
            if Utils.to_int(n1) not in closing_idxs: nt_to_interactions.setdefault(Utils.to_int(n1), []).append(i) 
            if Utils.to_int(n2) not in closing_idxs: nt_to_interactions.setdefault(Utils.to_int(n2), []).append(i) 
        used_ids = set() 
        for nt, bonds in nt_to_interactions.items(): 
            if len(bonds) >= 2: 
                counts["TRIPLET"] = counts.get("TRIPLET", 0) + 1 
                for b in bonds: used_ids.add(id(b)) 
        for i in valid_interactions: 
            if id(i) in used_ids: continue 
            if Utils.to_int(i["nt1"]) in closing_idxs or Utils.to_int(i["nt2"]) in closing_idxs: counts["EXTERNAL"] = counts.get("EXTERNAL", 0) + 1 
            else: counts["INTERNAL"] = counts.get("INTERNAL", 0) + 1 
        return counts 

    @staticmethod
    def categorize_junction_interaction(motif: Dict, interactions: List[Dict]) -> Dict[str, int]:
    ...
    @staticmethod 
    def categorize_bulge_interaction(motif: Dict, interactions: List[Dict]) -> Dict[str, int]: 
        ...
        valid_interactions = [i for i in interactions if not (Utils.to_int(i["nt1"]) in closing_idxs and Utils.to_int(i["nt2"]) in closing_idxs)] 
        nt_to_interactions = {} 
        for i in valid_interactions: 
            if Utils.to_int(i["nt1"]) not in closing_idxs: nt_to_interactions.setdefault(Utils.to_int(i["nt1"]), []).append(i) 
            if Utils.to_int(i["nt2"]) not in closing_idxs: nt_to_interactions.setdefault(Utils.to_int(i["nt2"]), []).append(i) 
        used_ids = set() 
        for nt, bonds in nt_to_interactions.items(): 
            if len(bonds) >= 2: 
                counts["TRIPLET"] = counts.get("TRIPLET", 0) + 1 
        ...
        for i in valid_interactions: 
            if id(i) in used_ids: continue 
            if Utils.to_int(i["nt1"]) in closing_idxs or Utils.to_int(i["nt2"]) in closing_idxs: counts["HELIX_ANCHOR"] = counts.get("HELIX_ANCHOR", 0) + 1 
            else: counts["INTRA_LOOP"] = counts.get("INTRA_LOOP", 0) + 1
        return counts
...
class Stacking:
    ...
    @staticmethod
    def _get_stacking_path_info(starts: List[int], targets: List[int], valid_nodes: Set[int], stacking_index: Set[frozenset]) -> Tuple[bool, int, List[int]]:
        """Breadth-first search to find the shortest stacking path between sets of nucleotides."""
        if not starts or not targets: return False, 0, []
        graph = {}
        ...
        loop_nodes_in_path = [n for n in path_nodes if n not in starts and n not in targets]
        return True, len(loop_nodes_in_path), full_path

    @staticmethod
    def _has_stacking_path(starts: List[int], targets: List[int], valid_nodes: Set[int], stacking_index: Set[frozenset]) -> bool:
        """Convenience method to check if a stacking path exists."""
        has_path, _, _ = Stacking._get_stacking_path_info(starts, targets, valid_nodes, stacking_index)
        return has_path
    ...
    def check_general_coaxiality(motif: dict, stacking_index: set) -> dict:
        ...
        s = next((s for s in motif.get("strands", []) if "." in s.get("structure", "")), None)
        if not s: return {"status": "ERROR", "metrics": {"size_2d": [0,0], "size_3d": [0,0], "stacking_path": []}}
        
        f, l = Utils.to_int(s.get("first")), Utils.to_int(s.get("last"))
        n_2d = s.get("structure", "").count(".")
        valid_nodes = set(range(min(f, l), max(f, l) + 1))
        
        has_path, stacked_count, path = Stacking._get_stacking_path_info([f], [l], valid_nodes, stacking_index)
        ...
        return {"status": status, "metrics": {"size_2d": [n_2d, 0], "size_3d": [stacked_count, 0], "stacking_path": path}}
    ...
    def check_internal_coaxiality(motif: dict, stacking_index: set) -> dict:
        ...
        strands = sorted(motif.get("strands", []), key=lambda x: Utils.to_int(x['first']))
        if len(strands) < 2: return {"status": "ERROR", "metrics": {"size_2d": [0,0], "size_3d": [0,0], "stacking_path": []}}
        
        f1, l1 = Utils.to_int(strands[0].get("first")), Utils.to_int(strands[0].get("last"))
        f2, l2 = Utils.to_int(strands[1].get("first")), Utils.to_int(strands[1].get("last"))
        ...
        valid_nodes.update(interface_up); valid_nodes.update(interface_down)

        success1, _, path1 = Stacking._get_stacking_path_info(interface_up, interface_down, valid_nodes, stacking_index)
        if not success1: return {"status": "NO", "metrics": {"size_2d": s2d, "size_3d": [0,0], "stacking_path": []}}

        start_used, end_used = path1[0], path1[-1]
        other_start = interface_up[1] if start_used == interface_up[0] else interface_up[0]
        other_target = interface_down[1] if end_used == interface_down[0] else interface_down[0]
        
        remaining_nodes = valid_nodes - set(path1[1:-1])
        success2, _, path2 = Stacking._get_stacking_path_info([other_start], [other_target], remaining_nodes, stacking_index)
        
        s3d = [len(path1)-2 if len(path1)>2 else 0, len(path2)-2 if len(path2)>2 else 0]
        
        if success2:
            return {"status": "FULL", "metrics": {"size_2d": s2d, "size_3d": s3d, "stacking_path": [path1, path2]}}
        else:
            return {"status": "PARTIAL", "metrics": {"size_2d": s2d, "size_3d": [s3d[0], 0], "stacking_path": [path1, []]}}
    ...
    def check_junction_stacking(motif: dict, stacking_index: set) -> dict: 
        ...
        n = len(strands)
        if n < 3: return {"status": "ERROR", "coaxial_pairs": []}
        valid_nodes = set()
        for s in strands:
            f, l = Utils.to_int(s.get("first")), Utils.to_int(s.get("last"))
            if f is not None and l is not None:
        ...
        for i, j in itertools.combinations(range(1, n + 1), 2):
            if Stacking._has_stacking_path(stem_ends[i], stem_ends[j], valid_nodes, stacking_index):
    ...
    def check_hairpin_stacking(motif: dict, stacking_index: set) -> dict:
        ...
        if f is None or l is None: return {"status": "ERROR", "metrics": {"size_2d": [0,0], "size_3d": [0,0], "stacking_path": []}} 
        return {"status": "SKIPPED", "metrics": {"size_2d": [n2d, 0], "size_3d": [n2d, 0], "stacking_path": [f, l]}}
...
class GeometryCalculator:
    ...
    def load_structure(path: str):
        ...
        if GeometryCalculator._loaded_structure == path: return
        cmd.reinitialize(); cmd.load(path, "full_struct"); GeometryCalculator._loaded_structure = path
    ...
    def extract_stem(pdb_path: str, selection_query: str, out_pdb_path: str):
        ...
        GeometryCalculator.load_structure(pdb_path); cmd.save(out_pdb_path, f"full_struct and ({selection_query})")
    ...
    def get_stem_selection(stem: Dict) -> Optional[str]:
        ...
        if not stem or not stem.get("strand5p") or not stem.get("strand3p"):
            return None
        s5p_f, s5p_l = stem["strand5p"].get("first", {}), stem["strand5p"].get("last", {})
        s3p_f, s3p_l = stem["strand3p"].get("first", {}), stem["strand3p"].get("last", {})
    ...
    def get_helical_axis(pdb_path: str) -> list:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(pdb_path, 'r') as s, open(tmp_pdb, 'w') as d: d.write(s.read())
            try:
                p1 = subprocess.Popen(f"find_pair {tmp_pdb} stdout".split(), stdout=subprocess.PIPE, cwd=tmpdir)
                p2 = subprocess.Popen("analyze".split(), stdin=p1.stdout, stdout=subprocess.PIPE, cwd=tmpdir)
            ...
            with open(out, 'r') as f:
                for line in f:
                    if line.startswith("Helix:"):
                        p = line.split()
                        if len(p) >= 4: return [float(p[1]), float(p[2]), float(p[3])]
        return None

    @staticmethod
    def get_stem_axis_cached(cif_path: str, stem_data: Dict, label: str = "tmp") -> Optional[List[float]]:
    ...
    @staticmethod
    def get_junction_bend_angles(cif_path: str, stems_data: Dict[str, Dict]) -> Dict[str, float]:
...
class Visualizer:
    ...
    def prepare_3d_components(data_obj: Dict, is_junction: bool = False) -> List[Dict]:
        ...
        def process_stems(target):
            if not is_junction:
                ...
                for s_t in ['upstream', 'downstream']:
                    ...
                    if res: comp3d.append({'id': s_t.upper(), 'type': 'STEM', 'residues': res})
            else:
        ...
        def process_components(target, palette, prefix=""):
            for i, c in enumerate(target.get('components', [])):
                ...
                if ires: comp3d.append({'id': f"{prefix}INTERNAL_STEM_{i+1}", 'type': 'STEM', 'residues': ires})
    ...
    def get_continuous_selection(data: Dict, is_junction: bool = False) -> str:
        ...
        if is_junction:
            for s in data.get("location", []): add_s(s)
            for stem in [v for k, v in data.get("context", {}).items() if "stem" in k]:
                add_s(stem.get("strand5p")); add_s(stem.get("strand3p"))
            for ext_h in data.get("extended_helices", []):
                ...
                for comp in ext_h.get("components", []):
                    for loc in comp.get("location", []): add_s(loc)
    ...
    @staticmethod
    def generate_pml_script(pml_path: str, cif_path: str, data: Dict, full_sel: str, is_junction: bool = False):
    ...
    @staticmethod
    def export_varna_json(data: Dict, out_path: str, mapping: Dict = None, is_junction: bool = False):
        """Exports the RNA secondary structure to a JSON format compatible with VARNA visualization."""
        nt_data = {}
        def add_s(s, color):
            if not s: return
            f, l, seq, struct = Utils.to_int(s.get("first")), Utils.to_int(s.get("last")), s.get("sequence", ""), s.get("structure", "")
            if f is None or l is None: return
        ...
        if is_junction:
            stacked = {s for p in data.get("modules", {}).get("stacking", {}).get("coaxial_pairs", []) for s in p}
            for ext_h in data.get("extended_helices", []):
                for k in ["upstream", "downstream"]:
                    add_s(ext_h.get("strands", {}).get(k, {}).get("strand5p"), "#D3D3D3"); add_s(ext_h.get("strands", {}).get(k, {}).get("strand3p"), "#D3D3D3")
                for comp in ext_h.get("components", []):
        ...
        for i, ser in enumerate(sorted_serials):
            ...
            nucleotides.append({"id": v_id, "number": lbl if counts[lbl] == 1 else str(ser), "char": nt_data[ser]['char'], "innerColor": nt_data[ser]['color'], "outlineColor": "black"})

        base_pairs, stack = [], []
        for ser in sorted_serials:
            if nt_data[ser]['struct'] == '(': stack.append(ser)
            elif nt_data[ser]['struct'] == ')' and stack:
                p_ser = stack.pop()
                base_pairs.append({"id1": ser_to_v_id[p_ser], "id2": ser_to_v_id[ser], "edge5": "WC", "edge3": "WC", "stericity": "CIS", "canonical": True})

        lw_s, lw_e = {'c': 'CIS', 't': 'TRANS'}, {'w': 'WC', 'h': 'HOOGSTEEN', 's': 'SUGAR'}
        interactions = []
        if is_junction:
            interactions.extend(data.get("modules", {}).get("interactions", {}).get("details", []))
            for ext_h in data.get("extended_helices", []):
                for comp in ext_h.get("components", []): interactions.extend(comp.get("interactions", []))
        else:
            for comp in data.get("components", []): interactions.extend(comp.get("interactions", []))
...
class HelicesBuilder:
    ...
    @staticmethod
    def get_global_angle(cif_path: str, motif_start: Dict, motif_end: Dict) -> Optional[float]:
        """Calculates the bend angle between the first and last stem of a super-helix."""
        ctx_start = motif_start.get("location", {}).get("context", {}).get("upstream", {})
        if motif_end.get("meta", {}).get("type") == "HAIRPIN":
            ctx_end = motif_end.get("location", {}).get("context", {}).get("upstream", {})
        else:
            ctx_end = motif_end.get("location", {}).get("context", {}).get("downstream", {})
        
        v1 = GeometryCalculator.get_stem_axis_cached(cif_path, ctx_start, label="tmp_g1")
    ...
    def stems_match(d: Dict, u: Dict) -> bool:
        ...
        try: return Utils.to_int(d.get("strand5p", {}).get("first")) == Utils.to_int(u.get("strand5p", {}).get("first")) and Utils.to_int(d.get("strand5p", {}).get("last")) == Utils.to_int(u.get("strand5p", {}).get("last"))
        except: return False

    @staticmethod
    def extract_helices(motifs: List[Dict]) -> List[Dict]:
        """Groups motifs into raw helix sequences based on stem connectivity."""
        v = [m for m in motifs if m.get("modules", {}).get("stacking", {}).get("status") in ["FULL", "BULGE-IN", "BULGE-OUT"] or (m.get("meta", {}).get("type") == "HAIRPIN" and m.get("modules", {}).get("stacking", {}).get("status") in ["PARTIAL", "FULL", "SKIPPED"])]
        if not v: return []
        v.sort(key=lambda x: Utils.to_int(x.get("location", {}).get("strands", [{}])[0].get("first")) or 0)
        h, cur = [], [v[0]]
        for i in range(1, len(v)):
            prev_down = cur[-1].get("location", {}).get("context", {}).get("downstream", {})
            next_up = v[i].get("location", {}).get("context", {}).get("upstream", {})
            if cur[-1].get("meta", {}).get("type") == "HAIRPIN" or not HelicesBuilder.stems_match(prev_down, next_up):
                h.append(cur); cur = [v[i]]
            else: cur.append(v[i])
        h.append(cur)
    ...
    def _format_helices(groups: List[List[Dict]]) -> List[Dict]:
        ...
        for idx, g in enumerate(groups, 1):
            if len(g) < 2: continue
            f, l = g[0], g[-1]
            s1f = f.get("location", {}).get("context", {}).get("upstream", {}).get("strand5p", {}).get("first")
            s1l = l.get("location", {}).get("context", {}).get("downstream", {}).get("strand5p", {}).get("last")
            s2f = l.get("location", {}).get("context", {}).get("downstream", {}).get("strand3p", {}).get("first")
            s2l = f.get("location", {}).get("context", {}).get("upstream", {}).get("strand3p", {}).get("last")
            total = (abs(Utils.to_int(s1l) - Utils.to_int(s1f)) + 1) + (abs(Utils.to_int(s2l) - Utils.to_int(s2f)) + 1) if all(x is not None for x in [Utils.to_int(s1f), Utils.to_int(s1l), Utils.to_int(s2f), Utils.to_int(s2l)]) else 0
            comps = []
            for m in g:
                ...
                comps.append({
                    ...
                    "metrics": {
                        ...
                        "stacking": m.get("modules", {}).get("stacking", {}).get("status"), 
                        "bend_angle": m.get("modules", {}).get("geometry", {}).get("bend_angle")
                    }, 
                    "interactions": m.get("modules", {}).get("interactions", {}).get("details", []), 
                    "internal_stem": m.get("location", {}).get("context", {}).get("downstream", {})
                })
            res.append({"h_id": idx, "total_nt": total, "strands": {"upstream": {"first": s1f, "last": s1l}, "downstream": {"first": s2f, "last": s2l}}, "components": comps})
        return res
